...
def get_orders_repository() -> OrdersRepository:
    def make_order(item):
        ...
        make_round = lambda x: Round(
            items=list(map(make_round_item, x.get("items"))), id=UUID(x.get("id"))
        )
...
def get_search_inventory_use_case(
    inventory_repository: Annotated[
        InventoryRepository, Depends(get_inventory_repository)
    ]
) -> SearchInventoryUseCase:
    return SearchInventoryUseCase(inventory_repository)


def get_all_inventory_use_case(
    inventory_repository: Annotated[
        InventoryRepository, Depends(get_inventory_repository)
    ]
...
def get_update_inventory_use_case(
    inventory_repository: Annotated[
        InventoryRepository, Depends(get_inventory_repository)
    ]
...
def get_all_orders_use_case(
    ...
    order_presenter: Annotated[OrderPresenter, Depends(get_order_presenter)],
) -> GetAllOrdersUseCase:
