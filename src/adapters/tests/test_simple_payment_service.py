...
def test_orders_items_calculation(payment_service: PaymentService):
  ...
  items = payment_service.calculate_order_items(
     rounds=[
      UpsertRoundModel(
         items=[
            RoundItem(product_id=product_id_a, price_per_unit=price_per_unit_a, quantity=1),
            RoundItem(product_id=product_id_b, price_per_unit=price_per_unit_b, quantity=2),
         ]
      ),
      UpsertRoundModel(
         items=[
            RoundItem(product_id=product_id_a, price_per_unit=price_per_unit_a, quantity=3),
         ]
      ),
      UpsertRoundModel(
         items=[
            RoundItem(product_id=product_id_b, price_per_unit=price_per_unit_b, quantity=3),
         ]
      )
    ],
  )
